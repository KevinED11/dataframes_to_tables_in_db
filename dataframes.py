...
def obtener_dataframes(archivo_excel: pd.ExcelFile, hojas_a_usar: list[str]) -> list[pd.DataFrame]:
    ...
    """
    dataframes: list[pd.DataFrame] = [pd.read_excel(archivo_excel, sheet_name=hoja,
                                                    header=2).dropna()
                                                    for hoja in hojas_a_usar]
    return dataframes
...
def dataframes_a_sql(dataframes: list[pd.DataFrame],
                     nombres_tabla: list[str],
                     conn: Connection) -> None:
    """Genera la consulta sql mediante pandas para convertir 
    los dataframes en tablas de base de datos"""
    for indice, dataframe in enumerate(dataframes):
        dataframe_sin_nd: pd.DataFrame = dataframe.replace("ND", 0.0, inplace=False)
        dataframe_sin_nd.to_sql(name=nombres_tabla[indice],
                                con=conn,
                                if_exists="replace",
                                index=False,
                                chunksize=100)
